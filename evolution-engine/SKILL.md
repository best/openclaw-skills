...
PCEC（Periodic Cognitive Evolution Cycle）是 OpenClaw 技能生态的自我进化机制。它周期性检测系统问题、诊断根因、提出进化方案——但所有修改操作必须经过审核后才执行。

**核心变化（v2 → v3）：** 从"自主修改 + 事后追踪"变为"证据链诊断 + 草案审核"。进化能力不变，治理方式变了。
...
> **职责边界说明**：PCEC / Dream / Heartbeat 的完整职责划分见 `references/responsibility-boundary.md`。
...
2. **review**：由 OpenClaw 主 session 或系统管理员审核，检查：
   - 铁律是否完整（证据链 / 只写草案 / 静默）
   - 草案格式是否规范
   - 排除范围是否合理
   - 是否包含个人化信息（公开仓库禁忌）
