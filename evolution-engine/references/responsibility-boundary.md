...
| 组件 | 定位 | 核心职责 | 频率 |
|------|------|---------|------|
| **PCEC** | Wiki-Native 进化引擎 | 信号检测→经验召回→诊断→**Wiki固化**（只写知识，不改代码） | 由调用方配置 |
| **Dream 🌙** | 记忆整合引擎 | T1 日志扫描 → T0 四动作 → T2 Wiki 维护 | 由调用方配置 |
| **Heartbeat** | 系统健康巡检 | Cron 运维面（存活/错误/资源）— 轻量高频 | 由调用方配置 |
...
|--------|---------|-------------------|------|
| **PCEC** | Gene（策略模板） | `Gene: <信号> — <策略名>` | `Gene: API超时重试 — 指数退避策略` |
| **PCEC** | Capsule（诊断记录） | `Capsule: YYYY-MM-DD <目标> <摘要>` | `Capsule: YYYY-MM-DD example-task timeout分析` |
